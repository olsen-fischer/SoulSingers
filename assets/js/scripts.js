...
if (videoBtn.length > 0) {
    videoBtn.forEach(element => {
        element.addEventListener('click', function (e) {
            ...
            // Remove '#' from target for document.getElementById()
            var targetId = videoDataBsTarget.replace('videoModal', '');
            videoDataBsTargetModale = document.getElementById(targetId);

            // --- When modal is shown ---
            videoDataBsTargetModale.addEventListener('shown.bs.modal', function () {
                // Optional autoplay
                iframeVideo.setAttribute('src', videoSrc + '?autoplay=1&modestbranding=1&showinfo=0');
            });

            // --- When modal starts closing ---
            videoDataBsTargetModale.addEventListener('hide.bs.modal', function () {
                // Add animation classes if any exist
                var modContent = this.querySelector('.modal-content');
                if (modContent) {
                    modContent.classList.remove('modal-animated-in');
                    modContent.classList.add('modal-animated-out');
                }
            });

            // --- When modal is fully closed ---
            videoDataBsTargetModale.addEventListener('hidden.bs.modal', function () {
                // Stop the video properly
                iframeVideo.setAttribute('src', '');
                iframeVideo.setAttribute('src', videoSrc);

                // Restore animation class for next open
                var modContent = this.querySelector('.modal-content');
                if (modContent) {
